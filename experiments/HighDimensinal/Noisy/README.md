...
For the new **100D noisy Hard/Tied study** at noise standard deviations 0.1,
0.2 and 0.8, see `README_NOISY.md` and run
`python3 experiments/exp_noisy_grouped_Rd.py`. Its completed results are in
`noisy_example_results/`. The description below concerns the original noiseless
entry point and its separate results.
...
To use your existing GitHub checkout, copy **all Python files** from this
package's `experiments/` into its `experiments/` folder. This replaces
`experiments/exp_mlp_Rd.py` and adds its helper modules and the noisy entry point. Run the same command
